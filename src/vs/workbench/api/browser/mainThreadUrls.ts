...
export class MainThreadUrls implements MainThreadUrlsShape {
	...
	async $createAppUri(extensionId: ExtensionIdentifier, options?: { payload?: IURLCreateOptions }): Promise<URI> {
		return this.urlService.create(extensionId.value, options ? options.payload : undefined);
	}
